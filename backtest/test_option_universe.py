...
def _print_summary(label: str, underlying: str, decision_date: datetime) -> None:
    ...
    if metadata_candidates:
        ...
        bars_request = OptionBarsRequest(
            symbol_or_symbols=[contract["symbol"] for contract in metadata_candidates],
            start=start_dt,
            end=end_dt,
    ...
    final_universe = get_historical_option_universe(underlying, decision_date)
    ...
    print(f"final historical universe: {len(final_universe)}")
    for contract in final_universe[:10]:
...
if __name__ == "__main__":
    ...
    _print_summary("AAPL historical universe validation", "AAPL", decision_date)
